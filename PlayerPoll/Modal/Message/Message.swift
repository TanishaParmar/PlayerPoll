...
internal struct MockMessage: MessageType {
    ...
    private init(kind: MessageKind, user: MockUser, messageId: String, date: Date, read: Bool = false) {
        self.kind = kind
        self.user = user
    ...
    init(text: String, user: MockUser, messageId: String, date: Date, read: Bool) {
        self.init(kind: .text(text), user: user, messageId: messageId, date: date, read: read)
    }
...
//        self.uid = snap.key
//        self.date = snap.childSnapshot(forPath: "date").value as? String ?? ""
//        self.dateTime = snap.childSnapshot(forPath: "dateTime").value as? String ?? ""
//        self.isSeen = snap.childSnapshot(forPath: "isSeen").value as? Bool ?? true
//        self.message = snap.childSnapshot(forPath: "message").value as? String ?? ""
//        self.sender = snap.childSnapshot(forPath: "sender").value as? String ?? ""
//        self.receiver = snap.childSnapshot(forPath: "receiver").value as? String ?? ""
//        self.time = snap.childSnapshot(forPath: "time").value as? String ?? ""
//        let dateString = date + " " + time
//        self.timestamp = dateString.getFormattedDate()
//    }
...
class MessageModal{
    static let currentUser:MockUser = MockUser(senderId: Globals.userId, displayName: "")
//    static let currentRestUser:MockUser = MockUser(senderId: Globals.getStoredRestPhone(), displayName: "")
//    static let supportUser:MockUser = MockUser(senderId: Globals.returnSupportNumber(), displayName: "")
}
